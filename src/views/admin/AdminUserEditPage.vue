<template>
  <div class="admin-user-edit-page p-4">
    <h1 class="text-2xl font-bold mb-4">사용자 정보 수정</h1>

    <div v-if="loading" class="text-center text-gray-500">사용자 정보를 불러오는 중...</div>
    <div v-else-if="error" class="text-center text-red-500">{{ error }}</div>
    <div v-else class="mb-6 p-4 border rounded-lg shadow-sm bg-white">
      <form @submit.prevent="handleUpdateUser">
        ...
        <BaseFormField label="역할">
          <select v-model="editingUser.role" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2">
        ...
        <BaseFormField label="생년월일">
          <BaseInput type="text" v-model="editingUser.birthdate" placeholder="YYYYMMDDHHmmss" />
          <p class="text-xs text-gray-500 mt-1">예: 19900101120000 (YYYYMMDDHHmmss)</p>
        </BaseFormField>
        ...
        <div class="flex space-x-2 mt-4">
          <BaseButton type="submit" class="rounded-md bg-black py-2 text-sm font-bold text-white hover:bg-gray-800">저장</BaseButton>
          <BaseButton type="button" @click="$router.back()" class="rounded-md bg-gray-300 py-2 text-sm font-bold text-gray-800 hover:bg-gray-400">취소</BaseButton>
        </div>
      </form>
    </div>
  </div>
