...
const { getTraineeAsset, assetData, errorMessage } = useTraineeAsset();
...
<template>
  <div
    class="relative flex min-h-screen flex-col bg-realBlack px-6 pb-20 pt-10"
  >
    <!-- Header -->
    <!--
    ...
    <div class="space-y-6 px-6">
      ...
      <div class="rounded-xl bg-gray-100 p-6 shadow-lg">
        <div class="flex items-start justify-between">
          <div>
            <h2 class="mb-2 text-heading font-semibold text-black">총 자산</h2>
            <p class="text-title font-bold text-black">{{ balance }} 원</p>
          </div>
          <button
            v-if="!isAssetFetched"
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div class="rounded-xl bg-gray-100 p-4 shadow-lg">
          ...
          <div class="flex h-32 items-center justify-center text-gray-700">
            <span class="text-subtext">막대 그래프</span>
          </div>
        </div>
      </div>

      <!-- Sticky AI Chat Button -->
      <div class="sticky bottom-2 flex justify-end pr-2 pt-10">
        <button
          @click="goToAiChat"
          class="w-15 h-15 flex items-center justify-center rounded-full bg-primary shadow-lg transition-transform hover:scale-105 active:scale-95"
        >
          <div class="relative">
            <svg
              width="30"
